...
public class Menu {
    ...
    public Menu(User userOnline, Date dataBase){
        this.menuArea(userOnline, dataBase);
    }

    // ----- Utilidade para o Construtor da Classe ----- //
    private void menuArea(User userOnline, Date dataBase){
        if(userOnline.getChoose()==4 || userOnline.getChoose()==5){
            ...
            while(control){
                ...
                switch(option){
                    case 0:
                        ...
                        break;
                    case 1:
                        this.profileArea(userOnline, dataBase);
                        break;
                    case 2:
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        this.projectsArea(userOnline, dataBase);
                        break;
                    case 5:
                        this.activitiesArea(userOnline, dataBase);
                        break;
                    default:
    ...
    private void profileArea(User userOnline, Date dataBase){
        ...
        switch(option){
            case 1:
                ...
                break;
            case 2:
                userOnline.printReport(dataBase);
                break;
            default:
                System.out.println("\nComando Invalido.");
                break;
    ...
    private void projectsArea(User userOnline, Date dataBase){
        boolean control = true;
        while (control){
            ...
            switch(option){
                case 0:
                    ...
                    break;
                case 1:
                    dataBase.addProject(userOnline);
                    break;
                case 2:
                    String nameProject = input.sourceProjects();
                    boolean controlCaseTwo = true;
                    for(Project p: dataBase.getProjectList()){
                        if(p.getIdProject().equals(nameProject) && p.getCoordinator().equals(userOnline)){
                                ...
                                case 5:
                                    ...
                                    for(User user: dataBase.getUserList()){
                                        if(user.getName().equals(name) && user.getLastName().equals(firstName)){
                                            ...
                                            p.addBag(userOnline, value,period);
                    ...
                    break;
                case 3:
                    dataBase.removeProject(userOnline);
                    break;
                case 4:
                    String nmProject = input.sourceProjects();
                    boolean ctrlEdit = true;
                    for(Project p: dataBase.getProjectList()){
                        if(p.getIdProject().equals(nmProject) && p.getCoordinator().equals(userOnline)){
    ...
    private void activitiesArea(User userOnline, Date dataBase){
        String idProject = input.sourceProjects();
        boolean c = true;
        for(Project p: dataBase.getProjectList()){
            if(p.getIdProject().equals(idProject) && p.getCoordinator() == userOnline){
                Boolean control = true;
                while(control){
